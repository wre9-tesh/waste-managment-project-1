"""
Version 2 of final_analysis.py - same computations, outputs prepared for figures that follow the
Nature Reviews artwork guide (see final_figures_v2.py).
Input : ../Locations of landfills in gujurat completer.kml
Outputs (all new; version 1 outputs are not touched):
  tables_v2/final_results_v2.xlsx and tables_v2/*.csv  - same tables and values as version 1
  tables_v2/table1_site_summary.csv / .xlsx            - publication-style Table 1 (replaces the fig13 table image):
                                                          full column names, units with superscript exponents (ha yr⁻¹)
  data/outlines_by_site_epoch_v2.geojson, data/outlines_by_part_v2.geojson
  data/results_v2.json                                 - version 1 content + display labels without abbreviations,
                                                          units, site order and figure grouping
Changes from version 1 (numbers are identical):
  - no 'LF' abbreviation in display labels ('Ahmedabad 1 (Pirana)', 'Surat 2 (Khajod)')
  - units written as ha, m, ha yr⁻¹, m yr⁻¹ (superscript exponents, not '/yr')
  - Table 1 exported as an editable table instead of an image
...
T = lambda *p: os.path.join(BASE, 'tables_v2', *p)
...
with pd.ExcelWriter(T('final_results_v2.xlsx')) as xw:
...
json.dump({'type': 'FeatureCollection', 'features': feats}, open(D('outlines_by_site_epoch_v2.geojson'), 'w'))
...
json.dump({'type': 'FeatureCollection', 'features': pf}, open(D('outlines_by_part_v2.geojson'), 'w'))
...
# ---- version 2: display labels, units, order, Table 1 ----
LABEL = {sid: re.sub(r'\bLF-(\d)', r'\1', n) for sid, n in NAMES.items()}          # 'Ahmedabad LF-1 (Pirana)' -> 'Ahmedabad 1 (Pirana)'
LABEL.update({sid: LABEL[sid].replace(' 1', '') for sid in LABEL                   # single-site cities: 'Rajkot'
              if sum(1 for s in NAMES if s[:3] == sid[:3]) == 1})
GROUP_LABEL = {'Major city': 'Five most populous cities', 'Other city': 'Other cities', 'All sites': 'All 14 sites'}
UNITS = {'area': 'ha', 'perimeter': 'm', 'area_rate': 'ha yr⁻¹', 'perimeter_rate': 'm yr⁻¹'}
ORDER = [s['site_id'] for s in sorted(summary, key=lambda s: (s['group'] != 'Major city', s['site_id']))]
t1 = [{'Site': LABEL[s['site_id']], 'City group': GROUP_LABEL[s['group']],
       'Area 2016 (ha)': round(s['area_2016_ha'], 1), 'Area 2026 (ha)': round(s['area_2026_ha'], 1),
       'Net change (%)': round(s['net_change_pct']), 'Largest area (ha)': round(s['peak_ha'], 1), 'Year of largest area': s['peak_year'],
       'Theil–Sen slope (ha yr⁻¹)': round(s['theil_sen_ha_per_yr'], 2), 'Mann–Kendall S': s['mk_S'],
       'Mann–Kendall exact P': round(s['mk_p_exact'], 3), 'Perimeter 2016 (m)': round(s['perimeter_2016_m']),
       'Perimeter 2026 (m)': round(s['perimeter_2026_m']), 'Trajectory class': s['trajectory']}
      for s in sorted(summary, key=lambda s: ORDER.index(s['site_id']))]
pd.DataFrame(t1).to_csv(T('table1_site_summary.csv'), index=False, encoding='utf-8-sig')
pd.DataFrame(t1).to_excel(T('table1_site_summary.xlsx'), index=False)
json.dump(dict(epochs=EPOCHS, names=NAMES, labels=LABEL, group_labels=GROUP_LABEL, units=UNITS, order=ORDER,
               rows=[rows[k] for k in sorted(rows)], parts=part_rows, summary=summary,
               transitions=trans, groups={g: dict(d) for g, d in grp.items()}, cities=city_rows, shapes=shapes,
               centroids=cent, log=log, overlaps=overlap_notes), open(D('results_v2.json'), 'w'), default=float)
